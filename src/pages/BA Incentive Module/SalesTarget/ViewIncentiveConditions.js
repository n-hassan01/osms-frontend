...
const TABLE_HEAD = [
  { id: 'incentive_type_id', label: 'Incentive Type Name', alignRight: false },
  { id: 'parent_incentive_type_id', label: 'Parent Incentive Type Name', alignRight: false },
];
const selectedUsers = [];
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => _user.incentive_type_id.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function ViewIncentiveDistributionAll() {
    ...
    try {
      if (exceldata && Array.isArray(exceldata)) {
        for (const row of exceldata) {
          const requestBody = {
            custGroupId: row.incentive_type_id,
          ...
          try {
            ...
            if (postData.status === 200) {
              console.log(`Row with incentive_type_id ${row.incentive_type_id} successfully added.`);
            } else {
  ...
  return (
    <>
      <Helmet>
        <title> Incentive Conditions | COMS </title>
      </Helmet>
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
          ...
          <div>
            ...
            <Button
              ...
              onClick={() => {
                navigate('/dashboard/addincentiveconditions');
              }}
            >
              Add Incentive Conditions
            </Button>
            {/* <Button
