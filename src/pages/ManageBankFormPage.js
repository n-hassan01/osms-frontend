...
// sections
import { getBankAllService, getUserProfileDetails } from '../Services/ApiServices';
...
export default function ManageBankFormPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let response = {};
        if (account) response = await getBankAllService(); // Call your async function here
  ...
  const TABLE_HEAD = [
    { id: 'bank_name', label: sentenceCase('bank_name'), alignRight: false },
  ...
  const addSo = () => {
    navigate('/dashboard/bankformpage', { replace: true });
  };
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { bank_id,bank_name,  description, address_line1, city } =
                      row;

                    return (
                      <TableRow hover key={bank_id} tabIndex={-1}>
                        <TableCell align="left">
                          <Link
                            onClick={() => {
                             
                                navigate(`/dashboard/updatebankformpage/${bank_id}`);
