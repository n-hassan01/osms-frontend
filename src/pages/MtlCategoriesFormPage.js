...
export default function MtlCategoriesFormPage() {
  ...
  const saveHeader = async () => {
    ...
    rows.forEach(async (lineInfo, index) => {
      const requestBody = {
        description: lineInfo.description,
        segment1: lineInfo.segment1,
        segment2: lineInfo.segment2,
        lastUpdateDate: date,
        lastUpdatedBy: account.user_id,
        lastUpdateLogin: account.user_id,
        creationDate: date,
      ...
      const response = await addmtlcategoriesfromService(requestBody, user);
      if (response.status === 200) {
        alert('Data Saved!');
  ...
  return (
    <>
      <Helmet>
        <title> COMS | MTL Categories</title>
      </Helmet>

      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={3}>
          <Typography variant="h4" gutterBottom>
            Create MTL Categories Form
          </Typography>
              ...
              <thead>
                <tr>
                  ...
                  <th style={{ width: '150px', textAlign: 'right' }}>
                    Segment1 <span style={{ color: 'red' }}>*</span>
                  </th>
                  <th style={{ width: '50px', textAlign: 'right' }}>
                    Segment2 <span style={{ color: 'red' }}>*</span>
                  </th>
                </tr>
              </thead>
              <tbody>
