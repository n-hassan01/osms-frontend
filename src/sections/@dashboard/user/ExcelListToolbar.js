...
export default function ExcelListToolbar({ numSelected, filterName, onFilterName, selectedUsers }) {
  const deleteSelectedUser = async () => {
    let alertMessage;
    const result = selectedUsers.map(async (element) => {
      try {
        const response = await axios.put(`http://182.160.114.100:5001/delete-hr-locations-all/${element}`);
